    ...
    <div className="min-h-screen bg-white text-black font-sans pt-24 pb-20">
      <main className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative h-[600px] bg-black border-4 border-black rounded-[2rem] overflow-hidden  mb-20">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <h2 className="text-white text-6xl font-[1000] uppercase italic tracking-tighter drop-shadow-2xl text-center px-10">
          ...
          {/* 轮播控制 */}
          <button onClick={() => setCurrentSlide((s) => (s - 1 + 3) % 3)} className="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 bg-white border-2 border-black rounded-full flex items-center justify-center hover:bg-[#98FF8A] transition-all z-10"><ChevronLeft/></button>
          <button onClick={() => setCurrentSlide((s) => (s + 1) % 3)} className="absolute right-6 top-1/2 -translate-y-1/2 w-12 h-12 bg-white border-2 border-black rounded-full flex items-center justify-center hover:bg-[#98FF8A] transition-all z-10"><ChevronRight/></button>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16 border-b-4 border-black pb-10">
          <div className="max-w-2xl">
            <h1 className="text-7xl font-[1000] uppercase italic tracking-tighter leading-none mb-4 text-black">
              生态保护 <span className="text-[#98FF8A] ">新闻</span>
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-24">
          {NEWS_LIST.map((news) => (
            <motion.div 
              key={news.id}
              whileHover={{ y: -8 }}
              className={`border-2 border-black bg-white  flex flex-col ${news.wide ? 'md:col-span-2' : ''}`}
            >
              <div className="h-64 border-b-2 border-black overflow-hidden relative">
                <img src={news.img} className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" alt="" />
                <div className="absolute top-4 left-4 bg-black text-white px-3 py-1 text-[10px] font-black uppercase tracking-tighter">
                  {news.category}
                </div>
              </div>
              <div className="p-6 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-2xl font-black italic uppercase leading-tight mb-4 group-hover:underline">{news.title}</h3>
                </div>
                <button className="flex items-center gap-2 font-black uppercase text-xs mt-4 hover:text-[#fff95e] transition-colors">
        ...
        {/* --- 4. 特色倡议区域 (Featured Initiative) --- */}
        <div className="border-4 border-black bg-[#C7FFBF] rounded-[3rem] p-10 md:p-16 mb-24 flex flex-col md:flex-row gap-12 ] relative overflow-hidden">
          ...
          <div className="flex-1 h-[400px] border-4 border-black rounded-[2rem] overflow-hidden rotate-2 shadow-xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-12 bg-black text-white p-12 rounded-[3rem] mb-20">
          <div className="max-w-xl">
            <span className="text-[#C7FFBF] font-black uppercase text-xs tracking-widest">Protect what matters</span>
          ...
          <div className="flex gap-10">
            <div className="text-center">
              <div className="text-4xl font-[1000] text-[#C7FFBF] italic">128K</div>
            ...
            <div className="text-center border-l border-white/20 pl-10">
              <div className="text-4xl font-[1000] text-[#C7FFBF] italic">8.2K</div>
